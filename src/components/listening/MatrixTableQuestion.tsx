...
const MatrixTableQuestion = observer(({
  ...
  const handleCellClick = (questionId: number, option: string) => {
    ...
    const currentAnswer = listeningStore.getAnswer(questionId) as string[] | string | undefined
    ...
    // Update answer
    listeningStore.setAnswer(questionId, answerArray)
  }

  // Check if a cell is selected
  const isCellSelected = (questionId: number, option: string): boolean => {
    const answer = listeningStore.getAnswer(questionId)
      ...
      >
        <thead>
          <tr>
            ...
            {listeningStore.isPreviewMode && listeningStore.mockId && listeningStore.sectionId && (
              <th
                className="py-2 px-3 text-center font-bold text-sm"
                style={{
                  border: '2px solid #d1d5db',
        ...
        <tbody>
          {questions.map((question) => {
            ...
            const submittedAnswer = listeningStore.isPreviewMode
              ? listeningStore.getSubmittedAnswer(question.id)
              : null
            const isCorrect = listeningStore.isPreviewMode
              ? listeningStore.isAnswerCorrect(question.id)
              : null

            // Determine row border color based on correctness
            let rowBorderColor = 'var(--border-color)'
            if (listeningStore.isPreviewMode && submittedAnswer) {
              if (isCorrect === true) {
            ...
            return (
              <tr key={question.id} className="hover:bg-opacity-50">
                ...
                <td
                  ...
                  style={{
                    border: '2px solid #d1d5db',
                    backgroundColor: listeningStore.isPreviewMode && submittedAnswer
                      ? (isCorrect === true ? '#dcfce7' : isCorrect === false ? '#fee2e2' : 'var(--secondary)')
                      : 'var(--secondary)',
                    color: 'var(--text-primary)',
                    borderLeft: listeningStore.isPreviewMode && submittedAnswer
                ...
                {options.map((option, optIdx) => {
                  ...
                  const wasSubmitted = listeningStore.isPreviewMode &&
                  ...
                  if (listeningStore.isPreviewMode && wasSubmitted) {
                    if (isCorrect === true) {
                  ...
                  } else if (isSelected && !listeningStore.isPreviewMode) {
                      ...
                      <div className="flex items-center justify-center min-h-[36px]">
                        {(isSelected || wasSubmitted) && (
                          <svg
                            ...
                            viewBox="0 0 24 24"
                            style={{
                              color: listeningStore.isPreviewMode
