...
const TableCompletionQuestion = observer(({ question, questionNumber, isPreviewMode = false }: TableCompletionQuestionProps) => {
    ...
    if (containerRef.current) {
      setTimeout(() => {
        ...
        inputs.forEach((input) => {
          ...
          if (placeholderNum === currentQuestionNumber && !htmlInput.disabled) {
            htmlInput.focus()
            htmlInput.select()
          }
        })
  ...
  }, [readingStore.currentQuestionIndex, readingStore.currentPart, readingStore.isPreviewMode, questionNumber])
